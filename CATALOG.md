...
The September 2026 collection contains **267 photographs, including 77 textiles**. There are 208 museum imports, 48 new field or archive photographs, and the original eleven studies. Egypt (93 photographs) and Pompeii and the Roman world (33) lead the expansion, alongside Greece, Mesopotamia and Persia. These curated groupings overlap. The wider selection includes Nubian and Ethiopian painting, San and Australian rock painting, Bagan, Sigiriya, Japanese masks, Tibetan paintings, Moche reliefs and painted Maya ceramics. Later historic works retain their individual dates. See the [search audit and remaining gaps](public/photo-search.md).
...
- `public/originals/` holds unmodified museum or source JPEGs. The 267 originals total approximately 1,119 MiB. The study JPEG can be smaller than a museum's archival master; record dimensions describe the actual packaged JPEG.
...
- `public/collections.json` records six curated groups, their covers and explicit membership. `data/featured-order.json` records the first gallery page.
...
`collection` accepts `egypt`, `rome`, `greece`, `mesopotamia`, `persia` or `textiles` and combines with the other filters. Unknown collection IDs return 400. Shared collection links use `#collection=egypt`, for example.
