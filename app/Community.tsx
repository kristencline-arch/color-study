...
export default function Community({onPhoto, onShare}: {onPhoto: (photo: CommunityPhoto) => void; onShare: () => void}) {
  ...
  async function publish(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const context = [["Subject", data.get("subject")], ["Object date (contributor supplied)", data.get("object_date")], ["Photographed", data.get("photo_date")]].filter(([, value]) => value).map(([label, value]) => `${label}: ${value}.`).join(" ");
      data.set("description", [context, data.get("description")].filter(Boolean).join("\n"));
      data.delete("subject"); data.delete("object_date"); data.delete("photo_date");
  ...
  return <section className="community-page">
    <div className="community-heading"><div><p className="eyebrow">THE OPEN PHOTO COLLECTION</p><h1>One more photo.<br /><em>One more way to see.</em></h1><p>Contribute your own photographs of textiles and painted surfaces: woven or dyed cloth, faded murals, cave art, painted plaster or traces of color on sculpture. Everyone can browse, download and experiment.</p></div><div className="community-heading-actions"><a className="button ghost" href="/api/dataset?download=all">Download the database <span aria-hidden="true">↓</span></a><button className="text-button" onClick={onShare}>Invite someone to contribute ↗</button></div></div>
    <div className="contribution-layout">
      ...
      <form className="contribution-form" ref={contributionForm} onSubmit={publish}>
        <fieldset disabled={publishing}>
          ...
          <div className="form-pair"><label>Photo title<input name="title" required maxLength={120} placeholder="A woven pattern, a painted wall…" /></label><label>Your photographer credit<input name="author" required maxLength={100} autoComplete="nickname" placeholder="Name or public credit" /></label></div>
          ...
          <div className="form-pair"><label>Subject <span className="optional">Optional</span><select name="subject"><option value="">Choose a subject</option><option>Painted wall or cave</option><option>Textile or dyed cloth</option><option>Painted sculpture or object</option><option>Manuscript or scroll</option><option>Painted pottery</option></select></label><label>Approximate object date <span className="optional">Optional</span><input name="object_date" maxLength={80} placeholder="For example: c. 500 BCE, or unknown" /></label></div>
          <label>When was the photograph taken? <span className="optional">Optional</span><input name="photo_date" type="date" /></label>
          <label>What should someone look for? <span className="optional">Optional</span><textarea name="description" maxLength={800} rows={3} placeholder="Describe the textile or painted surface, its date if known, lighting or conservation work." /></label>
    ...
    <section className="photo-guide" aria-labelledby="photo-guide-title"><p className="eyebrow">MAKE A USEFUL CONTRIBUTION</p><h2 id="photo-guide-title">Photograph the surviving color.</h2><div className="photo-guide-grid"><article><h3>Keep the color information</h3><p>Use the largest original color file you have. Avoid filters, screenshots, heavy compression and black-and-white conversions.</p></article><article><h3>Give the surface even light</h3><p>Keep the camera steady. Avoid glare and very dark shadows. Follow the site’s photography rules, including restrictions on flash.</p></article><article><h3>Include context and a detail</h3><p>A whole-object view helps place the image; a close-up helps inspect pigment. Record the site, object date if known, and any visible restoration.</p></article><article><h3>Respect the people connected to it</h3><p>Share photographs you have the right to publish. Consider local guidance about sacred or culturally sensitive material. A general location is enough.</p></article></div>
      <h3>Photographs we’re looking for</h3><p>Original painted surfaces at Angkor Wat; further Egyptian tomb and temple details; Herculaneum, Oplontis and Stabiae; painted manuscripts and textiles from underrepresented communities.</p><div className="contribution-requests">{["Angkor Wat, Cambodia", "Egypt", "Herculaneum, Italy", "Oplontis, Italy", "Stabiae, Italy", "Pacific Islands"].map(place => <button className="button ghost" key={place} onClick={() => {const form = contributionForm.current; const field = form?.elements.namedItem("location") as HTMLInputElement | null; if (field) field.value = place; form?.scrollIntoView({block: "start", behavior: "auto"}); (form?.elements.namedItem("title") as HTMLInputElement | null)?.focus();}}>Add a photo: {place}</button>)}</div>
    </section>
    ...
    <details className="remove-contribution"><summary>Remove a photograph you contributed</summary><p>Paste the contents of the private removal-key file you saved when publishing. Removal affects this site’s collection, not copies others already downloaded. To report someone else’s photo, use its private report form; no account is needed.</p><form onSubmit={remove}><label className="sr-only" htmlFor="removal-receipt">Your removal-key JSON</label><textarea id="removal-receipt" name="receipt" maxLength={500} rows={4} required placeholder='Paste your removal-key file contents here' /><button className="button ghost" disabled={removing}>{removing ? "Removing…" : "Remove my photograph"}</button></form>{removalStatus && <p role="status">{removalStatus}</p>}</details>
  </section>;
}
